...
import { getContact } from '../WeChatService';
import { ContactData, ContactItem, GetDataParams } from '../typing';
import toast, { Toaster } from 'react-hot-toast';

const columns = [
  { columnKey: "userName", label: "UserName" },
  { columnKey: "nickName", label: "NickName" },
  { columnKey: "alias", label: "Alias" },
  { columnKey: "isFriend", label: "IsFriend" },
...
export function ContactPage() {
  ...
  const { run } = useRequest<GetDataParams, ContactData>(params => getContact(params!));
  const [items, setItems] = React.useState<ContactItem[]>([]);
  const styles = useStyles();
  const dropdownId = useId("contact-dropdown-default");
  const options = [
    '10',
    '20',
    '30',
